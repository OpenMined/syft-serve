...
from syft_serve._exceptions import DuplicateServerError, ServerStartupError, PortConflictError
...
class TestCreateServer:
    ...
    def test_create_server_success(self, mock_config, sample_endpoints):
        ...
        with patch.object(manager, "_find_available_port", return_value=8000), patch.object(
            manager, "_start_server_process", return_value=12345
        ), patch.object(manager, "_wait_for_server_ready", return_value=True), patch.object(
            manager.servers, "add_server"
        ) as mock_add:

            server = manager.create_server(name="test_server", endpoints=sample_endpoints)
        ...
        with patch.object(manager.servers, "get_server", return_value=Mock()):
            with pytest.raises(DuplicateServerError):
                manager.create_server(
                    name="existing_server", endpoints=sample_endpoints, force=False
                )
    ...
    def test_create_server_duplicate_with_force(self, mock_config, sample_endpoints):
        ...
        with patch.object(
            manager.servers, "get_server", return_value=existing_server
        ), patch.object(manager.servers, "remove_server"), patch.object(
            manager, "_find_available_port", return_value=8000
        ), patch.object(
            manager, "_start_server_process", return_value=12345
        ), patch.object(
            manager, "_wait_for_server_ready", return_value=True
        ), patch.object(
            manager.servers, "add_server"
        ):

            server = manager.create_server(
                name="existing_server", endpoints=sample_endpoints, force=True
            )
    ...
    def test_create_server_startup_timeout(self, mock_config, sample_endpoints):
        ...
        with patch.object(manager, "_find_available_port", return_value=8000), patch.object(
            ...
            with pytest.raises(ServerStartupError, match="Server did not become ready"):
                manager.create_server(name="test_server", endpoints=sample_endpoints)
    ...
    def test_create_server_with_dependencies(self, mock_config, sample_endpoints):
        ...
        with patch.object(manager, "_find_available_port", return_value=8000), patch.object(
            ...
            manager.create_server(
                name="test_server", endpoints=sample_endpoints, dependencies=dependencies
            )
...
class TestServerProcessManagement:
    ...
    def test_wait_for_server_ready_timeout(self, mock_config):
        ...
        with patch("requests.get", side_effect=Exception("Connection failed")), patch("time.sleep"):
...
class TestTerminateServers:
    ...
    def test_terminate_all_servers(self, mock_config):
        ...
        with patch.object(manager.servers, "list_servers", return_value=servers), patch.object(
            manager.servers, "clear_servers"
        ):
    ...
    def test_terminate_all_with_orphaned_processes(self, mock_config):
        ...
        with patch.object(manager.servers, "list_servers", return_value=[]), patch.object(
            manager.servers, "clear_servers"
        ), patch(
            "syft_serve._process_discovery.discover_syft_serve_processes"
    ...
    def test_terminate_server_by_name(self, mock_config):
        ...
        with patch.object(manager.servers, "get_server", return_value=mock_server), patch.object(
            manager.servers, "remove_server"
        ):
...
class TestErrorHandling:
    ...
    def test_create_server_process_start_failure(self, mock_config, sample_endpoints):
        ...
        with patch.object(manager, "_find_available_port", return_value=8000), patch.object(
            manager, "_start_server_process", side_effect=Exception("Process failed")
        ):

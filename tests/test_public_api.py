...
@pytest.mark.skipif(
    not SYFT_SERVE_AVAILABLE,
    reason=f"syft_serve import failed: {IMPORT_ERROR if not SYFT_SERVE_AVAILABLE else ''}",
)
class TestPublicAPI:
    ...
    def test_server_instance_public_api(self, mock_manager_class):
        ...
        with patch("syft_serve._api._get_manager", return_value=mock_manager):
            server = ss.create(name="test_server", endpoints={"/test": lambda: {"message": "test"}})
...
class TestPublicAPIStructure:
    ...
    def test_internal_modules_renamed(self):
        ...
        expected_files = [
            "_api.py",
            "_config.py",
            "_endpoint_serializer.py",
            "_environment.py",
            "_exceptions.py",
            "_handle.py",
            "_log_stream.py",
            "_manager.py",
            "_process_discovery.py",
            "_server.py",
            "_server_collection.py",
        ]
